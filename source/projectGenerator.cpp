...
bool ProjectGenerator::passAllMake()
{
    ...
    for (const auto& file : staleFiles) {
        if (findFile(m_configHelper.m_rootDirectory + file, found)) {
            outputError("The source directory contains a '" + file + "' file generated by configure.");
            outputError(
                "These files conflict with the Visual Studio project generator and must be removed.", false);
            outputError("Please delete the following files from the source directory and try again:", false);
            for (const auto& f : staleFiles) {
                outputError("  " + m_configHelper.m_rootDirectory + f, false);
            }
            return false;
        }
    }
